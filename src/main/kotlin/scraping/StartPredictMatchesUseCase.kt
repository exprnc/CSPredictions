...
//    private val firestore by lazy { FirestoreClient.getFirestore() }
    ...
    private var needCookieCounter = 0
...
//        val serviceAccount = FileInputStream("cs-predictions-e54b2-firebase-adminsdk-is8f9-befa956987.json")
//
...
//    private suspend fun getFirestoreCookies() {
//        val cookieDocRef = firestore.collection("cookies").document("HLTV")
//        val currentCookie = withContext(Dispatchers.IO) {
//            cookieDocRef.get().get()
//        }.data
//        if (currentCookie != null) {
//            cookie = mapOf(Pair("__cf_bm", currentCookie.getValue("__cf_bm").toString()))
    ...
    private fun getCookie() {
        ...
        for (cookie in driver.manage().cookies) {
            if(cookie.name == "__cf_bm") {
                ...
                println("Current cookies: $cookies")
            }
        }
        driver.quit()
    }

    private suspend fun getDocument(url: String, from: Long = 7500, until: Long = 10000): Document {
        while (true) {
            try {
                if (needCookieCounter >= 5) {
                    getCookie()
                }
                showCurrentTime()
=======
